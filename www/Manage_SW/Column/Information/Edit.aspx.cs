...
public partial class Manage_SW_Column_Information_Edit : AdminRoot
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (managetype == 10148)
        {
            Size = "新闻（289*173px）视频（904*478px）";
        }
    }
    ...
    private List<Mod_PicList> SetPicList(Mod_Information MProduct)
    {
        ...
        if (albumArr != null)
        {
            ...
            for (int i = 0; i < albumArr.Length; i++)
            {
                ...
                if (imgArr.Length == 3)
                {
                    ls.Add(new Mod_PicList { ID = img_id, Model = modelArr[i], Title = TitleArr[i], ViceTitle = ViceTitleArr[i], ProductID = id, OriginalUrl = imgArr[1], ThumbUrl = imgArr[2], Info = remarkArr[i], State = 1, IsDefault = 0, WebSiteID = AdminManage.WebSiteID,OrderBy=i });
                }
            }
        }
        return ls;
    ...
    protected void btnEdit_Click(object sender, EventArgs e)
    {
        ...
        MessageBox.ShowRedirect(this, "信息保存成功！", "Column/Information/List.aspx?" + StringHelper.DelUrlParameter("ID"));
